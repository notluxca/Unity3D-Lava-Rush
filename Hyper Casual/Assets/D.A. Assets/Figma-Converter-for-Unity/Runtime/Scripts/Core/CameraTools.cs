...
namespace DA_Assets.FCU
{
    internal class CameraTools
    {
        ...
        {
            int layer = LayerTools.AddLayer(FcuConfig.BlurredObjectTag);
        ...
        {
            if (!IsTagExists(FcuConfig.BlurCameraTag))
            {
                AddTag(FcuConfig.BlurCameraTag);
            }

            Camera camera;
            GameObject bgBlurCamObj = GameObject.FindGameObjectWithTag(FcuConfig.BlurCameraTag);

            if (bgBlurCamObj == null)
            {
                GameObject cameraObject = new GameObject($"{FcuConfig.BlurCameraTag}Camera");
                camera = cameraObject.AddComponent<Camera>();
                cameraObject.tag = FcuConfig.BlurCameraTag;
            }
            else
            {
